...
若用弧长 $s$ 参数化平面曲线 $p(s)$，则 $\|p''(s)\| = \kappa(s)$（曲率）。当点列近似等弧长采样（$\Delta s$ 近似常数）时：
...
$$
\sum_i \|p_{i-1} - 2p_i + p_{i+1}\|^2 \;\approx\; \Delta s^3\!\int \kappa(s)^2\,\mathrm{d}s
$$

因此最小化二阶差分的平方和等价于最小化曲线的**弯曲能量（bending energy）**，使航向变化更连续、曲率更平滑，车辆转向更平顺。

3) **不是“拉直”，而是“去尖角”**

目标不是让路径变成直线——几何相似代价与位置偏移约束会把解拉回原始参考线附近。最终效果是“在不偏离道路的前提下，让每个局部三点的转角尽量不尖锐”。

> **为什么不直接惩罚曲率？** $\Delta^2 p_1$ 对坐标 $(x_i, y_i)$ 是线性的，$\|\Delta^2 p_1\|^2$ 是标准二次型，天然适配 QP 框架。若直接对转角/曲率做精确惩罚则会引入归一化与非线性项，优化问题更复杂。
**矩阵表示**：令
