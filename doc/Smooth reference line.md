...
设路径上依次有三个点 $p_0(x_0, y_0)$、$p_1(x_1, y_1)$、$p_2(x_2, y_2)$，如下图所示：
...
我们希望这三个点尽可能**共线**（即路径在 $p_1$ 处尽可能平滑、没有拐角）。观察向量：

$$
\vec{BA} = p_0 - p_1 = (x_0 - x_1,\; y_0 - y_1)
$$

$$
\vec{BC} = p_2 - p_1 = (x_2 - x_1,\; y_2 - y_1)
$$

当三点**完全共线**且 $p_1$ 位于线段 $p_0p_2$ 的**几何中点**（在离散点近似等间距采样时可近似理解为成立）时，$\vec{BA}$ 和 $\vec{BC}$ 方向相反且模长相等，因此它们的矢量和为零向量：$\vec{BA} + \vec{BC} = \mathbf{0}$。更一般地，仅“共线”并不必然推出 $\vec{BA} + \vec{BC} = \mathbf{0}$；此时 $\vec{BA} + \vec{BC} = p_0 - 2p_1 + p_2 = 2\bigl(\tfrac{p_0 + p_2}{2} - p_1\bigr)$，刻画的是 $p_1$ 相对相邻两点中点的偏移（离散二阶差分）。

**为什么用 $\|\vec{BA} + \vec{BC}\|^2$ 作为平滑代价？为什么希望三点“尽可能共线”？**

1) **几何直觉（去“折角”/去“锯齿”）**：
$\vec{BA} + \vec{BC} = p_0 - 2p_1 + p_2 = 2\bigl(\tfrac{p_0 + p_2}{2} - p_1\bigr)$。因此最小化 $\|\vec{BA} + \vec{BC}\|^2$ 等价于把 $p_1$ 往其两侧邻点的中点 $\tfrac{p_0+p_2}{2}$ 拉近——这会抑制局部的“折角”和左右摆动（典型的折线锯齿会产生较大的二阶差分）。

2) **与曲率/二阶导的关系（离散化的“弯曲能量”）**：
若用弧长 $s$ 参数化平面曲线 $p(s)$，则 $\|p'(s)\|=1$ 且 $\|p''(s)\|=\kappa(s)$（曲率）。当点列近似等弧长采样（$\Delta s$ 近似常数）时，有离散近似：

$$
p_{i-1} - 2p_i + p_{i+1} \;\approx\; p''(s_i)\,\Delta s^2
$$

于是

$$
\sum_i \|p_{i-1} - 2p_i + p_{i+1}\|^2 \;\approx\; \Delta s^4\sum_i \|p''(s_i)\|^2 \;\approx\; \Delta s^3\int \kappa(s)^2\,\mathrm{d}s
$$

这相当于在最小化一条曲线的“弯曲能量”（curvature squared / bending energy），得到的解会倾向于**航向变化更连续、曲率更平滑**，从而车辆转向更顺（减少突兀的转角变化/横向加速度跳变）。

3) **“尽可能共线”是局部、小曲率偏好，而不是强制成直线**：
规划/参考线的目标不是让整段都变直，而是让每个局部三点的转角尽量不要太尖锐（小曲率）。同时，本问题还有几何相似项与位置偏移约束把解拉回原始参考线附近，因此最终是“在不偏离道路的前提下变得更平滑”。

> **实现层面的原因**：$\vec{BA} + \vec{BC}$ 对坐标 $(x_i, y_i)$ 是线性的，$\|\vec{BA} + \vec{BC}\|^2$ 是标准二次型，因此可以自然落在 QP 框架里并用 OSQP 高效求解；若直接对转角/曲率做精确惩罚，通常会引入归一化与非线性项，优化问题会更复杂。

矢量和越大，说明 $p_1$ 处的"折弯"越剧烈。事实上，$\vec{BA} + \vec{BC} = p_0 - 2\,p_1 + p_2$ 正是**二阶差分**，它近似反映了路径在该点的离散曲率。因此，平滑代价定义为该和向量模的平方：

$$
\mathbf{v} = \vec{BA} + \vec{BC} = (x_0 + x_2 - 2x_1,\; y_0 + y_2 - 2y_1)
$$

$$
\mathrm{Cost}_{S,3} = \|\mathbf{v}\|^2 = (x_0 + x_2 - 2x_1)^2 + (y_0 + y_2 - 2y_1)^2
$$
